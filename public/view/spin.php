<link rel="stylesheet" href="<?= URL_P_V ?>css/spin.css?v=1.0.1">
...
<div style="margin-top: 25vh" class="d-flex w-100 align-items-center justify-content-center px-3">
    <div class="col-12 d-flex flex-column align-items-center gap-3">
        ...
        <button id="btn-spin" type="button" class="btn btn-danger border-2 border-warning text-warning shadow fw-bold px-4 animate__animated animate__heartBeat animate__infinite">
            Quay Thưởng
        </button>
    </div>
</div>
...
<!-- Modal -->
<div class="modal fade" id="modalCongratulate" tabindex="-1" aria-labelledby="modalCongratulateLabel" aria-hidden="true">
    ...
    <div class="modal-content bg-dark-40 text-light blur-6 border-light">
      ...
      <div class="modal-body">
        <div class="d-flex flex-column align-items-center justify-content-center gap-3">
            ...
            <div id="list-has-prize" class="d-flex flex-column gap-1 align-items-center">
            </div>
        </div>
      </div>
      <div class="modal-footer">
        <a href="/list-prize" class="btn btn-sm px-3 btn-dark text-light-60"><i class="bi bi-gift"></i> DS giải</a>
...
<script>
    ...
    function startSpinning(finalTarget, list) {
        ...
        }, 5000); // 5000ms = 5 giây quay tổng cộng
    }

    function showCongratulation(list) {
        const $listContainer = $('#list-has-prize');
        $listContainer.empty(); // Xóa dữ liệu cũ

        // Thêm danh sách trúng thưởng vào Modal
        list.forEach(item => {
            $listContainer.append(`<span>${item.name_guest.toString().padStart(4, '0')}</span>`);
        });

        // Khởi tạo và hiển thị Modal (Luôn mở cho đến khi tương tác)
